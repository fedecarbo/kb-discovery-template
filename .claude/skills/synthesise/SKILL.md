...
**First pass:**
- Intake has finished on a session: `synthesis.md` has the metadata + Transcription flags filled in, and the rest is empty / template.
- `00-context/sample-coverage.md`, `consent-log.md`, `glossary.md` have already been updated.

**Second pass:**
- A `<artifact-base>-review-<YYYY-MM-DD>.md` file exists sibling to any artifact (session synthesis, insight file, living artifact).
- The Headline + Action items in that review file describe what to reconcile.
...
- Individual `02-synthesis/insights/I-*.md` files that the INDEX suggests are in scope.
...
- **Update Status** — `hunch` → `emerging` → `strong` → `contested` per the README's promotion rules. Source count alone isn't enough; disconfirmation discipline applies.
- **Spawn a new insight file** if a claim doesn't fit any existing one. Use `_TEMPLATE-insight.md` as the starting structure.
...
- **Sample-bias flags travel with the status.** If two consecutive sessions come through the same channel and support the same cluster of insights, the cluster cannot graduate past `hunch` until a non-channel-routed session — record this in the insight's Status line and in `probes.md`.
...
- Every affected insight file has a new evidence row + status check.
...
- A handoff note to the Curator listing what they should regenerate next.
